		...
		if errMain.checkBit('gsmUsb'):                  # CHECK IF GSM IS DISCONNECTED FROM RPi
			...
			if flagConn=='Success':
				...
				if flagCheck == 'ErrorTimeout':
					errTime.setBit('at+cipstart="TCP","54.169.181.88","5000"')
				elif flagCheck == 'Error':
					errGsm.setBit('at+cipstart="TCP","54.169.181.88","5000"')
				else:
					errGsm.clearBit('at+cipstart="TCP","54.169.181.88","5000"')
					errUnknown.clearBit('at+cipstart="TCP","54.169.181.88","5000"')
	...
	def run(self):
		...
		while True:
			...
			if not arg:
				...
				flagConn = self.gsm.sendAt('at+cipstart="TCP","54.169.181.88","5000"','OK','FAIL')
